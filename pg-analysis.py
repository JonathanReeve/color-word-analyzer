...
conn = sqlite3.connect('./pg-text-7.db')
c = conn.cursor()
...
for idChunk in idChunks:
    ...
    command = f"./color-word-analyzer-cli --colorMap=XKCD {filesStr} > results-{chunkIndex}.html"
    # print('Running command: ', command)
    print(f'Analyzing {len(filesToAnalyze)} files in batch {chunkIndex} of {len(idChunks)}')
    os.system(command)
